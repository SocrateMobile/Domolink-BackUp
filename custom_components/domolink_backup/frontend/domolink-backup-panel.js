...
class DomoLinkBackupPanel extends HTMLElement {
  ...
  _startAutoRefresh() {
    this._refreshTimer = setInterval(() => {
      if (this._hass) this._fetchData(true);
    }, 10000);
  }
  ...
  async _triggerBackup() {
    if (!this._hass) return;
    const name = prompt("Nom de la sauvegarde (laisser vide pour automatique) :", "");
    if (name === null) return;
    try {
  ...
  async _testConnection() {
    ...
    this._render();
    try {
      const res = await this._hass.callWS({ type: "domolink_backup/test_connection" });
      this._fetchData();
    } catch (err) {
  ...
  _render() {
    ...
    const maxMb = d.max_storage_mb || 10240;
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .badge-status {
          ...
          font-size: 13px;
          font-weight: 600;
          background: ${connSuccess ? "rgba(16, 185, 129, 0.15)" : "rgba(239, 68, 68, 0.15)"};
          color: ${connSuccess ? "#34d399" : "#f87171"};
          border: 1px solid ${connSuccess ? "rgba(16, 185, 129, 0.3)" : "rgba(239, 68, 68, 0.3)"};
        }
        ...
        .badge-dot {
          ...
          height: 8px;
          border-radius: 50%;
          background: ${connSuccess ? "#10b981" : "#ef4444"};
          box-shadow: 0 0 8px ${connSuccess ? "#10b981" : "#ef4444"};
        }
        ...
        <div class="header">
          <div class="header-title-group">
            ...
            <div class="title-sub">
              ...
              <div class="app-subtitle">Stockage distant & haute sécurité pour Home Assistant • v1.0.1</div>
            </div>
          </div>
  ...
  _bindEvents() {
    ...
    if (btnBackup) btnBackup.onclick = () => this._triggerBackup();
