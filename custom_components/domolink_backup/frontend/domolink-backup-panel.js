...
class DomoLinkBackupPanel extends HTMLElement {
  ...
  _syncSidebarBadge(hasUpdate) {
    try {
      const isBackupUpdate = Boolean(hasUpdate);
      ...
      if (!this._sidebarObserverAttached) {
        ...
        try {
          const obs = new MutationObserver(() => this._syncSidebarBadge(hasUpdate));
  ...
  _bindEvents() {
    ...
    // Action buttons
    const appTitle = root.querySelector(".app-title");
    if (appTitle) {
...
function launchSocrateRulesEasterEgg(targetRoot) {
  ...
  const host = targetRoot.host || (targetRoot instanceof HTMLElement ? targetRoot : null);
